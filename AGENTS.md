...
- Supporting libraries:
  - `org.podval.xml` (`org.podval:org.podval.xml`, repo https://github.com/dubinsky/xml) —
    ...
    Local unreleased xml: optional `includeBuild` of sibling
    `../xml` (or `-PxmlDir=`).
...
- Site graph (opt-in `graph.enabled`, default off): `/graph.json` from `BackLinks` + `TransclusionEdges`; `/graph.html` loads
  Cytoscape only there.
...
- Links: wiki-style `[[...]]`, internal link resolution, backlinks, TOC. Markdown `![[note]]` / `![[note#heading]]` / `![[note#^block]]` transclude authored XML (`aside.transclusion`; Settings `transclusion-clean` is seamless). Media `![[file]]` stays `WikiLink.embed`. Missing `#fragment` is `UnresolvedTransclusion`, not a whole-page embed. `TransclusionEdge` harvest; transclude stubs are not backlinks; reverse list is **Embedded in**. Front-matter `permalink` / `aliases` are Refresh pages; if the target is a directory, `/permalink/child` and `[[permalink/child]]` resolve `child` under it (`Pages.find` longest alias prefix). TEI `store`/`collection` `@alias` is the collection short name (collector `site.xml` `<alias n= to=>`); it does not write a Refresh file. `Page.publishedPath` emits the short href; `serve()` rewrites it to the written file. `Path.fromHref` treats a last-dot suffix as an extension unless it is all digits (`255.2` stays one segment). Local media (`img@src`, `video`/`audio`/`source@src`, PDF `object@data`) go through `AssetRef` stamps and `Pages.resolveAsset` (published path, missing-asset errors), not page-link title-walk. TEI entity files are roots `person` / `place` / `org` (`EntityContent`); `persName` / `placeName` / `orgName` `@ref` resolves by matching kind and the entity file name without `.xml` (not title-walk). `entityLists` is `EntityListsContent` (kind+role buckets, members site-wide; catalog TOC or the single list; not a store include list); lists are generated at render so the catalog does not create backlinks.
...
- TEI `entityLists` is `EntityListsContent` (kind+role buckets, members site-wide). Beside `dir/` it is that directory's index. Two or more non-empty lists: catalog is TOC of `/dir/{n}.html`; one list: the catalog page is that list. Empty lists omitted. Identity Worker prefix on the catalog directory. No synthetic `/name.html`.
...
- The Errors page is written after other HTML so unknown citations and unresolved links found while rendering appear on it. Each kind section has `id` from `PageError.Kind.id` (`NoRef` → `#no-ref`). More than one kind: a TOC of those fragments. TEI `persName` / `placeName` / `orgName` without `@ref` (documents and store title/abstract/body, not entity-file names), TEI `unclear`, and entity file name ≠ underscored first name are recorded on the first parse of the source file (`PageError.NoRef` / `Unclear` / `MisnamedEntity`). There are no `/report` pages.
