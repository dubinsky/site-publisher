...
- The Errors page is written after other HTML so unknown citations and unresolved links found while rendering appear on it. Each kind section has `id` from `PageError.Kind.id` (`NoRef` → `#no-ref`). TEI `persName` / `placeName` / `orgName` without `@ref` (documents and store title/abstract/body, not entity-file names), TEI `unclear`, and entity file name ≠ underscored first name are recorded on the first parse of the source file (`PageError.NoRef` / `Unclear` / `MisnamedEntity`). There are no `/report` pages.
