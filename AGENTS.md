...
- Every real page writes its `textContent` (or copies assets). Standalone sidecar `asset: true` (`.yml` / `.yaml` next to a markup file) copies that file as an asset (no dialect process); internal `asset` and a directory `index` so marked are errors.
