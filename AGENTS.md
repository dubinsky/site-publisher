...
- Chunking (`chunk` / `chunk-depth` in front matter) and PDF are markup-independent. Site config `paginate-posts` (optional int) batches the synthetic `/posts` listing (`/posts.html`, `/posts/2.html`, …). Site config `home` (absolute path) makes `/index.html` a Refresh alias to that page (chunked TOC: `P/index.html`); do not also author `index.md`. Site config `facsimiles-url` (optional) is the facsimile JPEG base (collector `tei@facsimilesUrl`); unused until TEI facsimiles.
