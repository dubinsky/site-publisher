...
import org.podval.xml.{HtmlXmlWriterConfig, Xml, XmlWriterConfig}
...
final class PagedListSpec extends AnyFunSuite:
  ...
  test("nav: page 1 is newest, so Older goes forward and Newer back") {
    val first: String = (HtmlXmlWriterConfig: XmlWriterConfig).render(PagedList.nav(1, 3, i => s"/$i"))
    ...
    val last: String = (HtmlXmlWriterConfig: XmlWriterConfig).render(PagedList.nav(3, 3, i => s"/$i"))
