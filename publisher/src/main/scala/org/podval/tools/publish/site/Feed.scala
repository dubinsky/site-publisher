...
final class Feed(site: Site) extends SyntheticXmlAsset(site, Feed.path):
  ...
  override def xmlContent: Xml.Element =
    ...
    val prologue: Seq[Xml.Element] = Seq(
      ...
      element("title", site.config.title, typeAttr := "html"),
      element("subtitle", site.config.description),
      element("generator", Seo.generatorName, attr("uri") := Seo.generatorUrl),
      link(
        href := absoluteUrl(Feed.path),
        rel := "self",
        typeAttr := "application/atom+xml"
      ),
      link(
        href := s"${site.uri}/",
        rel := "alternate",
      ...
      element(
        "author",
        Seq(
          element("name", site.config.author),
          element("email", site.config.email)
        )
      )
    )

    val entries: List[Xml.Element] = posts.map(entry)
    element("feed", xmlns := atomNs, prologue, entries)
  ...
  private def entry(page: Page): Xml.Element =
    ...
    element(
      "entry",
      Seq(
        element("title", page.title, typeAttr := "html"),
        link(
          href := url,
          rel := "alternate",
          typeAttr := "text/html",
          title.map(titleAttr := _)
        ),
        element("published", published),
        element("updated", updated),
        element("id", absoluteUrl(page.publishedPath.withoutHtml)),
        element("author", Seq(element("name", author(page))))
      ) ++
        tags(page).map(tag => element("category", attr("term") := tag)) ++
        ...
        page.description.map(text => element("summary", typeAttr := "html", Xml.cdata(text))).toSeq ++
