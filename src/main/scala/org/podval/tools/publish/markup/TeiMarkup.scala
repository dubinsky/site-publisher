...
// Binding hints for XmlCodec are @Modifier.config(XmlCodec.Attribute, "") etc.
// Schema.derived ignores @xmlAttribute / @xmlNamespace (they are not Modifier).

//@Modifier.config(XmlCodec.NamespaceUri, "http://www.tei-c.org/ns/1.0")
//@Modifier.config(XmlCodec.NamespacePrefix, "tei")
