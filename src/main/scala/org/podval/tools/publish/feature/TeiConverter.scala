...
*/
final class TeiConverter extends Converter:
  override def convert(
    ...
    import TeiConverter.{copyAttribute, renameElement}
    
    val name: String = element.getName

    val result: Xml.Element = element.setAttributes(element.getAttributes.map((name, value) =>
      val nameNew =
        if !TeiConverter.reservedAttributes.contains(name)
        then name
        else TeiConverter.withPrefix(name)
      (nameNew, value)
    ))

    name match
      case "head" | "body" | "title" | "div" | "p" =>
        renameElement(TeiConverter.withPrefix(name), result)
      ...
      case "pb" =>
        renameElement("a", result.setText(TeiConverter.facsimileSymbol))

      // TODO tooltips on dates and gaps

      case _ => element
      
private object TeiConverter:
