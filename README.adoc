...
Pin the plugin version in `build.gradle`. Until a site applies the plugin, GitHub Actions can still
call `dubinsky/site-publisher/.github/actions/generate@v0.1.0` (pin the action tag; optional
`version` input overrides that action checkout’s `gradle.properties`).
