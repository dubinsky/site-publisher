...
A marker in a table cell is published as a lowercase letter with the list under that table.
...
Markdown:: CommonMark inline `` `code` `` and fenced blocks.
  ...
  Fenced `+++```mermaid+++` diagrams are native in Obsidian too (https://help.obsidian.md/syntax#Diagram[Help: Diagram]); this publisher loads Mermaid for `language-mermaid`.
+
[source,markdown]
----
Use `map` in
