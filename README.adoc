...
`persName`, `placeName`, and `orgName` with a non-empty `@ref` resolve to an entity when the kinds match and `@ref` is exactly that id (not a path, not the displayed name, not a title). A kind mismatch or unknown id is an unresolved internal link and is recorded on the Errors page. Without `@ref`, the name is left as-is (not a link) and is recorded on the Errors page (`name without @ref`). That check runs on the first parse of TEI documents and of store/collection `title` / `abstract` / `body` — not on the name elements that define an entity file. TEI `unclear` is recorded as `unclear`. An entity file whose name is not the first name with spaces turned into underscores is `misnamed entity`.
...
Generate also writes `collection-aliases.json` (segment `from` → collection directory `to`) for a Cloudflare Worker. The Worker is an internal rewrite (not a 301): longest slash-delimited prefix, then `.html` unless the request already has an extension (keep `.xml`); inbound `/alias/facsimile/P` becomes `{to}/P/facsimile.html`. The same table includes inbound `/name` (all-entities page) and `/name/{id}` (entity under the `entityLists` directory). Prefix routes (`hostname/rgada*`, `hostname/name*`, …) so assets skip the Worker. Deploy with `.github/actions/deploy-alias-worker`. Canonical entity hrefs stay `/names/{id}.html`.
