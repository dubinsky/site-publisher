...
`/graph.html` is a pan/zoom map of canonical authored pages and the resolved internal links between them (Cytoscape.js, loaded only on that page).
...
Aliases, chunks, facsimile viewers, PDFs, assets, and synthetics (`/errors`, `/tags`, `/posts`, …) are not vertices; links to those views count as links to the owning page.
Edges come from existing backlinks (`kind: link`, including TEI `@ref`) and, unless turned off, transclusions (`kind: transclude`).
...
A notes vault typically enables the graph and excludes the Obsidian daily-notes folder (`days` when that is the configured folder).
