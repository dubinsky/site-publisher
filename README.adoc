...
- `pdf` - see <<pdf>>
- `asset` — copy the markup file as an asset (no dialect processing, no site chrome). Standalone sidecar only: `name.yml` / `name.yaml` next to `name.html` (or `.md`, `.adoc`, `.xml`, …) with `asset: true`. The sidecar is not published. Internal `---` `asset: true` is an error (the file stays markup). A directory `index` cannot be an asset.
