...
Maven coordinates: `org.podval.tools:org.podval.tools.publisher:0.3.0`
...
----
plugins {
  id 'org.podval.tools.site-publisher' version '0.3.0'
}
site {
...
plugin (`org.podval.tools:org.podval.tools.publisher:0.3.0`).
...
- checks <<external-links>> when `check-links` is set;
...
- Front matter `post: true` keeps the file path and adds a Refresh page at `/YYYY/MM/DD/{file name}.html`.
  `date` is required.
  `post-title` replaces the file name in that path.
- Front matter `permalink: /YYYY/MM/DD/title` does the same for any source file name.
  The Refresh page is that permalink.
...
Footer social links, the license link, and facsimile JPEGs are not checked.
Generation still finishes.
The Errors page is the report, as for unresolved links.
Timeouts are 5 seconds to connect and 10 seconds per request.
...
TEI:: definition and use are the same element at the point of reference (`@n`, when non-blank, is the marker):
+
[source,xml]
----
...
----
+
A non-blank `@n` is that marker.
The next note with no `@n` is still numbered in series.
+
[source,xml]
----
...
DocBook:: `<footnote>` at the call site (`xml:id` optional; non-blank `label` is the same marker).
