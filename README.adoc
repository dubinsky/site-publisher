...
plugin (`org.podval.tools:org.podval.tools.publisher:0.2.0`).
...
TODO after a release that includes `asset: true` is on Maven Central: bump `sitePublisherVersion` in
`~/CognoMath/mathworlds-site` if needed and push that repo (`index.yml` copies `index.html`).
Do not push MathWorlds before then; CI resolves the publisher from Central.
...
Omitted or empty means no header links (up/prev/next and format icons still appear).
Unknown entries are recorded on the Errors page.
...
This here is a static site generator.
It:
...
- DocBook (`.xml` with a DocBook root: `article`, `book`, `chapter`, `appendix`, `part`, `set`, `preface`, `refentry`,
  `topic`)
...
Obsidian treats internal YAML as https://help.obsidian.md/properties[Properties]; it stores extra keys but does not
...
- `description`, `author`, `lang` — page `<head>` (<<seo>>); site values in `_site_config.yml` are the fallback.
  Site `lang` also selects store and selector display names (publisher `Selector.xml`, TEI `<name lang>`); omitted is
  `en`.
- `tags`, `categories`, `aliases`
- `icon`, `permalink`, `post`, `date`
- `pdf` - see <<pdf>>
- `asset` — copy the markup file as an asset (no dialect processing, no site chrome).
  Standalone sidecar only: `name.yml` / `name.yaml` next to `name.html` (or `.md`, `.adoc`, `.xml`, …) with
  `asset: true`.
  The sidecar is not published.
  Internal `---` `asset: true` is an error (the file stays markup).
  A directory `index` so marked stays a directory page (parent/children) but `write` copies the file (no listing, no
  chrome).
...
My plugin uses both the front matter title and the file name for link resolution, so I can remove this Obsidian plugin
and redundant front matter titles which are the same as the file name, and set the title in the front matter only when
it is different from the file name.

Obsidian uses file names exclusively and ignores the front matter title; for link resolution to use front matter titles
also, TODO Obsidian plugin needs to be installed.
...
Wiki `+++[[alter-rebbe]]+++` still finds the entity page by file name.
...
- `@role` present: entities of that kind whose root `@role` matches, anywhere in the site; omitted: entities of that
  kind with no role;
...
<<facsimiles,facsimile viewer>> when `facsimiles-url` is set.
...
A TEI `<date when="…">` (and the same on a date range written in `@when` with `..`) gets a hover table of the day in
Julian (when used), Gregorian, and Jewish calendars.
Month names follow the site `lang`.

`@when` is ISO-ish: `YYYY`, `YYYY-MM`, `YYYY-MM-DD`.
A year or month is an interval (first day .. last day).
An explicit range uses `..`; the right-hand side may be shorter (`1800-11-15..16`, `1800-11..12`).
...
Dates without `@when` are unchanged.
Invalid `@when` is a page error; the element is left as authored.

The collection index and document-header Дата cell show `@when` as the visible text (not the authored wording).
In-text dates keep their authored content.
...
`P.html` is the unsplit document (wiki `[[P]]` lands there).
...
When both views exist, the site header has icon links between them (list for chunked, file for one-page).
...
Markdown:: Kramdown https://kramdown.gettalong.org/converter/html.html#automatic-table-of-contents-generation[`{:toc}`]
  on a list.
  The list (seed item included) is replaced.
...
AsciiDoc `footnote:[…]` is written at the call site (including in a cell); the published page still lists those notes
under the table.
...
AsciiDoc:: inline note, or a named definition reused with an empty macro:
  A `footnote:[…]` in a table cell is written at the call site; the published page lists it under the table.
+
[source,asciidoc]
----
...
Markdown:: FlexMark: use `[^id]` in the text; define `[^id]:` later.
  Obsidian supports that form natively (https://help.obsidian.md/syntax#Footnotes[Help: Footnotes]).
  Obsidian also has inline `^[…]` footnotes; this publisher does not.
  A `[^id]` in a table cell uses the same `[^id]:` body at the file end; the published page moves it under the table.
+
[source,markdown]
----
...
TEI:: definition and use are the same element at the point of reference (`@n` is ignored):
+
[source,xml]
----
...
DocBook:: `<footnote>` at the call site (`xml:id` optional).
  Reuse with `<footnoteref linkend="id"/>`.
  A `<footnote>` inside another footnote's `<para>` is nested the same way if the converter emits an inner stub.
+
[source,xml]
----
...
A glossary is a description list marked as such.
Term links get hover definitions (print inlines them in parentheses).

AsciiDoc:: `[glossary]` on the description list; term ids from `[[id]]`.
...
Markdown:: PHP Extra definition lists (https://michelf.ca/projects/php-markdown/extra/#def-list) via FlexMark
  https://github.com/vsch/flexmark-java/wiki/Definition-List-Extension[Definition List Extension].
  Mark the list with `{:.glossary}` after it (Kramdown
  https://kramdown.gettalong.org/syntax.html#inline-attribute-lists[IAL], same family as <<toc>>).
  Without that marker the list is not a glossary (no term ids, no tooltips).
  The marker also separates adjacent lists.

Obsidian does not render PHP Extra definition lists natively (https://www.markdownguide.org/tools/obsidian/[Markdown
Guide: Obsidian]).
Community plugin https://obsidian.md/plugins?id=pandoc-extended-markdown[Pandoc Extended Markdown] renders Pandoc/PHP
...
TEI:: `<list type="gloss">` (or `type="glossary"`) of `<label>` / `<item>`.
  Term id from `xml:id` on the label, else on the item, else the label text with spaces turned into hyphens.
  Reference a term with `<ref target="#id">` or `<term ref="#id">`.
+
[source,xml]
----
...
DocBook:: `<glosslist>` (or `<glossary>`) of `<glossentry>` / `<glossterm>` / `<glossdef>`.
  Term id from `xml:id` on the entry, else the term, else the term text with spaces turned into hyphens.
  Reference a term with `<link linkend="id">`.
  `<variablelist>` is a description list, not a glossary.
+
[source,xml]
----
...
Two kinds, per document (no site-wide file or style).
They can appear in the same document.

=== External (`.bib` + CSL)

Front matter:

- `bibliography` — path to a `.bib` file, relative to this source file (`.bib` files are not copied to the site;
  un-ignore with `_site_ignore` if you want one published)
...
AsciiDoc:: no `asciidoctor-bibliography` gem.
  Also `cite:key[]` / `cite:key[p.
  12]` (standard inline-macro form).
+
[source,asciidoc]
----
...
Markdown:: Pandoc citation syntax (no extra FlexMark extension).
  `me@host.com` is left as an email.
  Obsidian does not format `[@key]` natively.
  Community plugin https://obsidian.md/plugins?id=obsidian-pandoc-reference-list[Pandoc Reference List] recognizes
  Pandoc
  citekeys in the note (sidebar; optional inline). https://obsidian.md/plugins?id=obsidian-citation-plugin[Citations]
  inserts `[@key]` from a `.bib` file.
+
[source,markdown]
----
...
DocBook:: `<citation>key</citation>` (optional locator after a comma).
  `biblioref` `@linkend` that is not a native entry id is the same.
  Empty `<bibliography/>` is the list placeholder (otherwise the list is appended).
+
[source,xml]
----
...
TEI:: `@cRef` on `ref` / `ptr` is the citeproc key.
  `@n` is an optional locator.
...
TEI:: `ref` / `ptr` `@target` the `xml:id` of a `bibl` / `biblStruct` in `listBibl` (not in `teiHeader`).
  Empty `ptr` shows `@n` or the id.
  `cit` with a `quote` is a <<quotes,quote>>, not this.
...
DocBook:: `<bibliography>` with `biblioentry` / `bibliomixed`; cite with `<link linkend="id">` or
  `<biblioref linkend="id"/>` (empty `biblioref` shows `@xreflabel` or the id).
+
[source,xml]
----
...
Markdown:: CommonMark inline `` `code` `` and fenced blocks.
  Obsidian supports both natively, including a language identifier (https://help.obsidian.md/syntax#Code[Help: Code]).
  Fenced `+++```mermaid+++` diagrams are native in Obsidian too
  (https://help.obsidian.md/syntax#Diagram[Help: Diagram]); this publisher loads Mermaid for `language-mermaid`.
+
[source,markdown]
----
Use `map` in
...
TEI:: `<code>` (tagdocs module) with `@lang` for the formal language (not `xml:lang`).
  A newline in the content is treated as a block and wrapped in `<pre>`.
  `<eg>` / `<egXML>` are not converted.
+
[source,xml]
----
...
DocBook:: `<code>` / `<literal>` inline; `<programlisting>` / `<screen>` / `<literallayout>` as a block.
  `@language` (not `xml:lang`) is the highlight.js tag.
+
[source,xml]
----
...
Numbered annotations on lines of *verbatim* (listing, literal, or source), plus a list of those annotations.
Not a general footnote mechanism.

AsciiDoc:: `<1>` (or `<.>` for automatic numbering) at the end of a line; repeat the numbers in a callout list below.
  Hide a marker in the raw source behind a line comment (`// <1>`, `# <1>`, `<!--1-->`).
+
[source,asciidoc]
------
[source,ruby]
...
DocBook:: `<co>` in a listing; `<calloutlist>` / `<callout arearefs>` below.
  `@label` on `co` is the marker number.
+
[source,xml]
----
...
Typed notices (note, tip, warning, …).
Not listing <<callouts>> and not asides/sidebars.

Markdown:: Obsidian core https://help.obsidian.md/callouts[callouts] (`> [!type]`; optional title; `+`/`-` to fold).
  No plugin.
+
[source,markdown]
----
...
AsciiDoc:: `NOTE:` / `TIP:` / `IMPORTANT:` / `CAUTION:` / `WARNING:` (paragraph or `[NOTE]====` block; optional
           `.Title`).
+
[source,asciidoc]
----
...
Untyped auxiliary content next to the main flow.
Not an <<admonitions,admonition>>.
...
Markdown:: no native syntax; Obsidian has none.
  Write an HTML `+++<aside>+++` block.
+
[source,markdown]
----
<aside>
...
AsciiDoc:: `[quote]` (or a `____` block); optional `.Title`; attribution and citation title as
           `[quote, Author, Source]`.
+
[source,asciidoc]
----
...
Markdown:: a CommonMark blockquote (`>`).
  No attribution syntax (an em-dash line is still quote body).
  Obsidian `> [!type]` is an <<admonitions,admonition>>, not a quote.
+
[source,markdown]
----
...
DocBook:: `<blockquote>` or `<epigraph>`; optional `title` and `attribution`.
  Inline `<quote>` stays `<q>`.
+
[source,xml]
----
<blockquote>
...
HTML:: `<blockquote>` (IR adds `class="quote"` if missing); optional title and `<footer>` attribution (`cite` kept
  as-is):
+
[source,html]
----
...
TEI:: `<quote>`; `<cit>` groups a quote with `<bibl>` (attribution).
  A `<bibl>` directly in `<quote>` is attribution too.
  `<q>` stays inline.
+
[source,xml]
----
...
DocBook:: `<emphasis role="strikethrough">` (or `line-through`).
  Default `<emphasis>` is `<em>`; `role="bold"` is `<strong>`.
+
[source,xml]
----
...
Markdown:: a `![alt](src)` on its own line; optional `"title"` is the caption.
  FlexMark does not turn Obsidian `|WIDTH` / `|WIDTHxHEIGHT` in the alt into `img` `width`/`height` (the size stays in
  `alt`).
  Obsidian `+++![[image]]+++` embeds stay `<img>` (resolved later); `|WIDTH` or `|WIDTHxHEIGHT` on that wiki embed do
  become `width`/`height`.
+
[source,markdown]
----
...
AsciiDoc:: block `image::` (optional `.Title`).
  Inline `image:src[]` is not a figure.
  Width and height (`image::file.png[alt, 320, 240]` or `width=320, height=240`) are kept on the `<img>`.
+
[source,asciidoc]
----
.A figure caption
...
TEI:: `<figure>` with `<graphic url>` and optional `<head>` (caption).
  `<graphic>` becomes `<img>`.
+
[source,xml]
----
...
A PDF file shown on the page.
Not <<pdf,PDF generation>> of a page.

Markdown:: Obsidian `+++![[file.pdf]]+++`; optional `#page=N` and `&height=H` (pixels if H is digits).
  Alias `+++![[file.pdf|Title]]+++` is the link label.
+
[source,markdown]
----
![[sample.pdf]]
...
Markdown:: Obsidian `+++![[file.mp4]]+++` (also `webm`, `ogv`, `m4v`).
  Alias `+++![[file.mp4|Title]]+++` is the fallback link label.
+
[source,markdown]
----
![[clip.mp4]]
...
AsciiDoc:: `video::file.mp4[]` (optional `.Title` becomes a figure caption).
  YouTube/Vimeo: `video::id[youtube]` / `video::id[vimeo]`.
+
[source,asciidoc]
----
video::clip.mp4[]
...
DocBook:: `<videodata fileref>`.
  A YouTube/Vimeo URL becomes an iframe.
+
[source,xml]
----
<mediaobject>
