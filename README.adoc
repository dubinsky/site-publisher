...
`../../Podval/site-publisher`. When the Gradle plugin exists, that include moves into
`pluginManagement { includeBuild(...) }` (one include, plugin + library; see
`gradle-plugin-plan.md`). Do not use `mavenLocal()` to dogfood the plugin.
