...
plugin always passes; a newer one is usually fine. A local `includeBuild` of this repo substitutes
...
A site that applies `org.podval.tools.site-publisher` substitutes a local checkout with **one**
include under `pluginManagement` (plugin id + library). Do not also `includeBuild` the same
directory in the settings body. Do not use `mavenLocal()`.
