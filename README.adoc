...
`--log-level` defaults to `INFO`. `--treat-errors-as-warnings`, `--include-drafts`, `--production`,
and `--target-directory-name` are the other flags. `SITE_PUBLISHER_*` environment variables are the
same names with hyphens turned into underscores.

`./gradlew build` compiles and tests; `./gradlew test` runs tests only.

Maven coordinates: `org.podval.tools:org.podval.tools.publisher:0.2.0`

[#gradle-plugin]
=== Gradle plugin

A site with Gradle applies plugin id `org.podval.tools.site-publisher` (same version as the
library). The plugin adds a detached `sitePublisher` configuration and two `JavaExec` tasks that
run `org.podval.tools.publish.site.Site`. Do not put the library on `implementation`: Scala version
and Playwright stay off the project compile classpath, and off the Gradle daemon.
...
`pluginManagement.repositories` must include `mavenCentral()` (the plugin marker is published
there) as well as `gradlePluginPortal()` for other plugins such as Foojay.

`./gradlew generateSite` writes the target directory. `./gradlew serveSite` is the same with
`--serve`. Do not make `generateSite` a dependency of `build`; Pages CI calls it explicitly.
...
Both tasks set `--enable-native-access=ALL-UNNAMED`,
`--sun-misc-unsafe-memory-access=allow`, `PLAYWRIGHT_SKIP_VALIDATE_HOST_REQUIREMENTS=1`, and
`PLAYWRIGHT_BROWSERS_PATH=$GRADLE_USER_HOME/ms-playwright`. `generateSite` launches Java 25.
Its inputs are the source tree minus the target directory and Gradle `build/`; its output is the
target directory, so Gradle up-to-date can skip a run when sources are unchanged. The generator
still deletes and rewrites the target when the task runs.

The plugin adds the library as a **default** dependency of the `sitePublisher` configuration, at
the same version as the plugin (`org.podval.tools:org.podval.tools.publisher:0.2.0`). Gradle
`defaultDependencies` run only when that configuration is still empty, so a consumer can pick
another published library version by declaring `sitePublisher` themselves (there is no
`site.publisherVersion`). Do not add a second coordinate alongside the default — a non-empty
configuration replaces the default entirely:
...
The supported pair is the same version as the plugin. An older library may not accept flags the
plugin always passes; a newer one is usually fine. A local settings-body `includeBuild` of this repo substitutes
the library **module** for whatever version string was requested.

Site workflows: checkout, setup-java 25, setup-gradle, cache `~/.gradle/ms-playwright`,
`./gradlew generateSite` (or `:docs:generateSite`), upload `_site` / `docs/_site`, deploy Pages.
Pin the plugin version in `build.gradle`. Known sites apply the plugin and call `./gradlew generateSite`
(or `:docs:generateSite`). GitHub Actions can still call
`dubinsky/site-publisher/.github/actions/generate@v0.1.0` for repos that do not apply the plugin
(pin the action tag; optional `version` input overrides that action checkout’s `gradle.properties`).

Sites using this publisher:

- https://dub.podval.org
- https://www.podval.org
- https://www.chumashquestions.org
- https://www.alter-rebbe.org
- https://www.opentorah.org
- https://www.mathworlds.org

[#dogfooding]
=== Dogfooding an unreleased publisher

This repo already `includeBuild`s sibling `../xml` and `../../OpenTorah/opentorah.org` when those
checkouts exist (`-PxmlDir=` / `-PopentorahDir=`).

A site that applies `org.podval.tools.site-publisher` substitutes a local checkout with **two**
`includeBuild`s of the same directory: `pluginManagement` (plugin id; pass a **String**, not a
`File`) and the settings body (library child `org.podval.tools:org.podval.tools.publisher`).
`pluginManagement` alone does not substitute the library now that it is not the included-build
root. Do not use `mavenLocal()`.
...
`../../Podval/site-publisher`. When the directory is missing (CI), Gradle resolves the plugin and
the library from Maven Central.

Without a site-side plugin, generate from this repo:
`./gradlew run --args="/path/to/source-directory"`.
...
The zip includes the library (`org.podval.tools:org.podval.tools.publisher`), the plugin
implementation (`org.podval.tools:site-publisher-plugin`), and the plugin marker
(`org.podval.tools.site-publisher:org.podval.tools.site-publisher.gradle.plugin`). Same version.
Plugin Portal: `./gradlew :site-publisher-plugin:publishPlugins`.

Then release the Central deployment at https://central.sonatype.com/publishing/deployments .

TODO after a release that includes `asset: true` is on Maven Central: bump `sitePublisherVersion` in `~/CognoMath/mathworlds-site` if needed and push that repo (`index.yml` copies `index.html`). Do not push MathWorlds before then; CI resolves the publisher from Central.
...
Empty lists are omitted. Members are sibling entity files in that directory, sorted by file name; the link text is the first `persName` / `placeName` / `orgName`. The entity page `<h1>` / `<title>` is that first name, not the file name. Document backlinks on an entity page still come from `@ref` in other documents (grouped by TEI collection when the source sits under one); the index itself is not a backlink source.

A synthetic `/name.html` lists every entity (display name, sorted). Inbound `/name` and `/name/{id}` rewrite to that list and to the entity file (`/names/{id}.html` when the file lives there). Emitted hrefs stay the file path.
...
Generate also writes `collection-aliases.json` (segment `from` → collection directory `to`) for a Cloudflare Worker. The Worker is an internal rewrite (not a 301): longest slash-delimited prefix, then `.html` unless the request already has an extension (keep `.xml`); inbound `/alias/facsimile/P` becomes `{to}/P/facsimile.html`. The same table includes inbound `/name` (all-entities page) and `/name/{id}` (entity under the `entityLists` directory) and `/report` / `/report/…`. Prefix routes (`hostname/rgada*`, `hostname/name*`, …) so assets skip the Worker. Deploy with `.github/actions/deploy-alias-worker`. Canonical entity hrefs stay `/names/{id}.html`.
...
A TEI `<date when="…">` (and the same on a date range written in `@when` with `..`) gets a hover table of the day in Julian (when used), Gregorian, and Jewish calendars. Month names follow the site `lang`.

`@when` is ISO-ish: `YYYY`, `YYYY-MM`, `YYYY-MM-DD`. A year or month is an interval (first day .. last day). An explicit range uses `..`; the right-hand side may be shorter (`1800-11-15..16`, `1800-11..12`).

The numbers in `@when` are Gregorian unless:

- the element has `calendar="#julian"`, or
- `_site_config.yml` has `tei-default-calendar: julian` and there is no `@calendar`.

Any other `@calendar` value is Gregorian. Dates without `@when` are unchanged. Invalid `@when` is a page error; the element is left as authored.

The collection index and document-header Дата cell show `@when` as the visible text (not the authored wording). In-text dates keep their authored content.
...
When the site has TEI entities or documents, `/report.html` lists apparatus reports (not in `header-pages`): names without `@ref` (`/report/no-refs.html`), TEI `unclear` (`/report/unclears.html`), and entity files whose id is not the underscored first name (`/report/misnamed-entities.html`). Harvest walks entity files, documents, and store title/abstract/body — not generated collection or name-list tables. Inbound `/report` and `/report/{id}` rewrite like collection aliases.
