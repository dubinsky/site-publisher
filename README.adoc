...
Maven coordinates: `org.podval.tools:org.podval.tools.publisher:0.1.0`

A site with Gradle should run that artifact on a detached `sitePublisher` configuration
(`JavaExec`, main class `org.podval.tools.publish.site.Site`). Do not put it on `implementation`
(Scala version and Playwright stay off the project compile classpath). A Gradle plugin that owns
this block is planned; see `gradle-plugin-plan.md`.

Until that plugin exists, GitHub Actions call
`dubinsky/site-publisher/.github/actions/generate@v0.1.0`, which resolves the same coordinate from
Maven Central (pin the action tag; optional `version` input overrides `gradle.properties`).
...
A site that depends on `org.podval.tools:org.podval.tools.publisher` can substitute a local
...
Override with `-PsitePublisherDir=`. OpenTorah sites typically want
`../../Podval/site-publisher`. When the Gradle plugin exists, add
`pluginManagement { includeBuild(...) }` as well (see `gradle-plugin-plan.md`).

Without a site-side dependency, generate from this repo:
`./gradlew run --args="/path/to/source-directory"`.
